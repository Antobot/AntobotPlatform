...
class teleop_joystick:
    def __init__(self):
        ...
        self.cmd_vel = Twist()
        self.max_lin_speed = 0.888
        self.max_ang_speed = 0.888
    ...
    def joy_callback(self, msg):
        ...
        if (self.buttons[self.button_lb] == 1 and self.buttons[self.button_rb] == 1):
            ...
            # This will remain true until the input times out
            # TODO - add a manual deactivate button to the joystick?
            
        
        elif (self.axes[self.axis_rt] == 0 or self.axes[self.axis_rt] == 1):
        # If the user is not holding down the right trigger
            
            if (self.buttons[self.button_a] == 1 and self.buttons_previous[self.button_a] == 0):
                self.trigger_resume=True # Send resume
                self.trigger_active=False

            elif (self.buttons[self.button_y] == 1 and self.buttons_previous[self.button_y] == 0):
                self.trigger_pause=True # Send pause

            elif (self.buttons[self.button_x] == 1 and self.buttons[self.button_back] == 1): #
                self.trigger_uvSwitch=True # Toggle UV state


            elif (self.buttons[self.button_b] == 1 and self.buttons[self.button_back] == 1): #
                self.trigger_camSwitch=True # Toggle camera state

            elif (self.buttons[self.button_start] == 1 and self.buttons_previous[self.button_start] == 0): #
                self.trigger_recSwitch=True # Toggle record
            ...
            elif (self.axes[self.axis_dpad_back_forward] == -1):  # dpad backward
        ...
        if (self.buttons[self.button_back] == 1 and self.buttons[self.button_a] == 1): #
            self.trigger_recoverJob=True # Reload previous job 
        ...
        # Limit the linear velocity based on angular velocity. - should match webUI

        # self.cmd_vel.linear.x = self.v*math.cos(self.axes[self.axis_analog_right]*3.14159/2)
