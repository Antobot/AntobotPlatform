...
# # #                       If main command, sends Twist commands to ROS publisher. Called by teleop.py                              [m, ,, .]]
...
################################################################################################
    ...
    def update(self):
        ...
        if (now-self.lastUpdate)>self.timeOut:
            ...
            # Check if the key is for a function instead
            #elif key == '0':
            #        self.mode_pub.publish(0) # Start's keyboard
            #if key == '3':
            #        self.mode_pub.publish(3) # Goes back to automated mode
            #if key == '5':
            #        self.mode_pub.publish(10) # pub row following
            #if key == '\x03':
            #    os.system("rosnode kill master_teleop")


            # functions can be set here based on other key inputs
            #self.trigger_pause=False
            #self.trigger_resume=False
            #self.trigger_abort=False
            #self.trigger_goHome=False
        ...
        # Update cmd_vel
        self.cmd_vel.linear.x = self.control_speed
        #self.cmd_vel.linear.y = 0
        #self.cmd_vel.linear.z = 0
        #self.cmd_vel.angular.x = 0
        #self.cmd_vel.angular.y = 0
        self.cmd_vel.angular.z = self.control_turn
